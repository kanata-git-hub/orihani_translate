...
async function main() {
  const {values} = parseArgs({options: {audio: {type: 'string'}, synthetic: {type: 'boolean'}, source: {type: 'string'}, target: {type: 'string'}, provider: {type: 'string'}, run: {type: 'boolean'}, 'openai-first': {type: 'boolean'}}});
  ...
  if (!values.run) {
    console.log(JSON.stringify({status: 'preflight_only', providerFilter: values.provider ?? 'both', durationMs: wav ? readWav(wav).durationMs : null, inputKind: values.synthetic ? 'synthetic' : 'provided_audio', sampleText: values.synthetic ? SYNTHETIC_CASES[values.source].text : undefined, openaiKeyAvailable: Boolean(key), willCallPaidApis: false}));
    return;
  }
